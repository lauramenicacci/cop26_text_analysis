...
words_list %>% 
  ...
  head(10) %>% kbl() %>% kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = F, fixed_thead = T) %>% save_kable("FrequencyTable.html")

png("FrequencyTable.png")
ggsave("FrequencyTable.png", width = 5, height = 3)
...
```{r, include=FALSE}
dygraph(timeseries, main = "All sentiments across time", xlab = "Date", ylab = "Frequency") %>%  #the plot, unfortunately, the dates are lost so far
