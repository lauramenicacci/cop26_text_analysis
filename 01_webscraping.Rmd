...
c26_toks <- cop26$Headlines %>% 
  corpus() %>% 
  tokens()
boris_toks <- boris$Headlines %>% 
  corpus() %>% 
  tokens()

sent_c26 <- tokens_lookup(c26_toks, dictionary = data_dictionary_LSD2015[1:2]) %>% 
  dfm() %>% 
  convert(to = "data.frame") %>% #targeted sentiment analysis for the boris words
  mutate(Type = "Cop26")
  
sent_boris <- tokens_lookup(boris_toks, dictionary = data_dictionary_LSD2015[1:2]) %>% 
  dfm() %>% 
  convert(to = "data.frame") %>% #targeted sentiment analysis for the boris words
  mutate(Type = "Boris")
