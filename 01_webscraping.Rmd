...
ifelse(sentiment_complete$sentiment == "negative", sentiment_complete$n == -n, n)

sentiment_complete %>% #first plot of sentiment analysis
  drop_na() %>% 
...
corpus_cop26 <- cop26$Headlines %>% 
  corpus() 
  
docvars(corpus_cop26, field = "date") <- floor_date(docvars(cop26$Dates, field = "Year"))
...
toks_neg <- tokens_keep(toks_cop26, pattern = phrase(neg), window = 10)

toks_neg_lsd <- tokens_lookup(toks_cop26, dictionary = data_dictionary_LSD2015[1:2]) #targeted sentiment analysis for the most negative words
