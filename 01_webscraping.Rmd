...
words_list %>% 
      ...
      save_kable("3_1FrequencyTable.html")
...
posnegtimeseries <- dygraph(timeseries2, main = "Positive and negative sentiments across time", xlab = "Date", ylab = "Frequency") %>% 
  dySeries("negatives", label="Negative") %>% 
  dySeries("positives", label="Positive") %>%
  dyOptions(labelsUTC = TRUE, fillGraph=TRUE, fillAlpha=0.5, drawGrid = FALSE) %>%
  dyRangeSelector() %>%
...
scatter_result_boris<- plot_ly(counter_boris_afinn, x = ~Dates, y = ~value, color = ~value, size = ~n, colors=colors ,sizes = c(10,40),
        ...
        marker = list(symbol = 'circle', sizemode = 'diameter',
                      line = list(width = 1, color = 'lightgrey')),
        text = ~paste('Count of Words', n), alpha = 1.2,
        width = 750, height = 450) #setting interactions
...
htmlwidgets::saveWidget(partial_bundle(scatter_result_boris), "6_3AffineAnalysisBoris.html")
