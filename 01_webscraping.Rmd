...
```{r}
#take most frequent neg words
boris_subset <- cop26 %>% filter(str_detect(Headlines, "Boris Johnson")|str_detect(Headlines, "Boris")|str_detect(Headlines, "Johnson"))


corpus_cop26 <- boris_subset$Headlines %>% 
  corpus() 

corpus_cop261 <- cop26$Dates %>% 
  corpus() 

try_c <- cbind(corpus_cop26, corpus_cop261)
  
#docvars(try_c, field = "Date") <- floor_date(docvars(try_c, field = "Date"))
...
#head(docvars(corpus_cop26))

neg <- c("limit", "failure", "protest")

toks_neg <- tokens_keep(toks_c, pattern = phrase(neg), window = 10)

toks_neg_lsd <- tokens_lookup(toks_c, dictionary = data_dictionary_LSD2015[1:2]) #targeted sentiment analysis for the most negative words

#create doc level variable for date
dfm_neg_lsd <- dfm(toks_neg_lsd)
...
sent_negative <- words_list %>% #we have 293 classified words
  filter(!word == "crisis") %>% #remove the word crisis before the sentiment analysis
  inner_join(get_sentiments("bing")) %>% 
...
#do it both negative and positive SO COOOOL also NAs have a role
sentiment_complete %>% 
  #drop_na() %>% 
  count(Dates, sentiment) %>% 
  group_by(sentiment) %>%
  ggplot(aes(x=Dates, y=n, color = sentiment)) +
  geom_line() +
  facet_wrap(~ sentiment, scales = "free_y") +
  theme_minimal() +
  ylab("Frequency of Negative Words during COP26")+
